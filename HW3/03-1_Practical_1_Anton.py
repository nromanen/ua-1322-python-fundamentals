...
#Convert all to upper for easy count() + displaying text in uppercase
zen = zen.upper()

#Find and count
print(f"'Better' count = {zen.count('BETTER')}",
      f"'Never' count = {zen.count('NEVER')}",
      f"'Is' count = {zen.count('IS')}",
      sep = "\n")

#Display in uppercase
print(f"\n {zen}")

#Replace all occurances of the sumbol "i" with "&"
for ind, x in enumerate(zen):
    if x == "I":
       zen = zen[:ind] + "&" + zen[ind+1:]
